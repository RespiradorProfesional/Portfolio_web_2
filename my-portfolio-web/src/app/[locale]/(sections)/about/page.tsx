...
export default async function AboutPage() {
  ...
  const technologiesGrouped = technologies.reduce<Record<string, Technology[]>>((acc, tech) => {
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center bg-[url('/background_images/background_ball.png')] bg-cover bg-center bg-no-repeat min-h-screen">
      ...
      <div>
        {Object.entries(technologiesGrouped)
        .map(([classification, techList]) => (
          <section key={classification} className="my-8 w-full max-w-4xl">
            <h2 className="text-2xl font-bold text-center mb-4">{classification}</h2>
      ...
      <div className='bg-background-2'>
        {courses.map((course) => (
          <LinkCard key={course.id} title={course.title} technologies={course.technologies} link={`/course/${course.id}`} />
        ))}
      </div>
